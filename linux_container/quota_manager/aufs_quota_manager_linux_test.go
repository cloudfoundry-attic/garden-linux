...
import (
	...
	"github.com/cloudfoundry-incubator/garden-linux/linux_container/quota_manager/fake_aufsdiffpathfinder"
...
var _ = Describe("AufsQuotaManager", func() {
	var (
		...
		fakeAUFSDiffPathFinder *fake_aufsdiffpathfinder.FakeAUFSDiffPathFinder
		mountDir               string
		backingFile            *os.File
		logger                 lager.Logger
	)
	...
	Describe("GetUsage", func() {
		...
		BeforeEach(func() {
			fakeAUFSDiffPathFinder = new(fake_aufsdiffpathfinder.FakeAUFSDiffPathFinder)

			quotaManager = &quota_manager.AUFSQuotaManager{fakeAUFSDiffPathFinder}
		...
		Context("when the rootFSPath exists", func() {
			It("correctly delegates the call to the AUFSDiffPathFinder", func() {
				_, err := quotaManager.GetUsage(logger, mountDir)
				Expect(err).NotTo(HaveOccurred())
