...
var _ = Describe("When nested", func() {
	...
	FIt("can start a nested garden-linux and run a container inside it", func() {
	...
	FIt("returns helpful error message when depot directory fstype cannot be nested", func() {
		...
		Ω(err).Should(MatchError("Cannot use /tmp/overlays as an overlay directory, directory is already mounted as overlayfs or aufs"))
	})
})
